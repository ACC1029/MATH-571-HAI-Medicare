...
# Change Not Available rows to NA
hosp_gen_info_reduced <- hosp_gen_info_reduced %>%
...
hosp_gen_info_reduced %>%
  filter(is.na(safety_of_care)) %>%
  select(provider_id)

df <- tibble(x = c("Above the national average", "Same as the national average", "Below the national average"))

df_recoded <- df %>%
  mutate(recoded_col = recode(x, 
